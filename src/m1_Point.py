...
#
#   Run the program and correct any s]yntax (notational) errors.
# ------------------------------------------------------------------------------
...
class Point(object):
    ...
    def move_by(self, dx, dy):
        self.total_dist = self.total_dist+((dx) ** 2 + (dy) ** 2) ** (0.5)
        self.x = self.x + dx
        self.y = self.y + dy
    ...
    def get_distance_from_start(self):
        dist = math.sqrt((self.x0 - self.x) ** 2 + (self.y0 - self.y) ** 2)
        return dist

    def move_to(self, x, y):
        self.total_dist = self.total_dist + ((self.x - x) ** 2 + (self.y - y) ** 2) ** (0.5)
        self.x = x
        self.y = y
        self.moves = self.moves + 1
    ...
    def halfway_to(self, p2):
        npoint = Point((self.x + p2.x) / 2, (self.y + p2.y) / 2)
        return npoint
